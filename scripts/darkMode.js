...
//Events
//see css to know more! wrote for root variables colors
//also documentElement points to root element(html)
//with this events goes to dark mode
toggleBtn.addEventListener('click', () =>
  document.documentElement.classList.toggle('dark-theme')
);
//Funcs
//now to create some posts
const articlesData = articles.map((value) => {
  //create multi variables with single obj property assgin to each
  const { title, date, length, snippet } = value;
