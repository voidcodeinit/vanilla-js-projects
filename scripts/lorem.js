...
form.addEventListener("submit",e=>{
  ...
  // Note*: our arrey length is 9
  //console.log(text.length);
  if (isNaN(value)|| value<0 || value>9) {
    result.innerHTML=`<p class="result">${text[random]}</p>`
  }
  else{
    //strat point and until the end point not the same end trime/slice other things(indexes)
    let tempText = text.slice(0, value);
    //console.log(tempText);
    //also i think you can use foreach also you can't use join outside of the map cuz it won't
    //join it to be just a string not an array
